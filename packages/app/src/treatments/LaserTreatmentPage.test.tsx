...
describe('LaserTreatmentPage', () => {
  ...
  const renderPage = (procedureId = mockProcedure.id, patientId = mockPatient.id) => {
    return render(
      <MedplumProvider medplum={medplum}>
        <MemoryRouter initialEntries={[`/Patient/${patientId}/laser-treatment?procedureId=${procedureId}`]}>
          <Routes>
            <Route path="/Patient/:id/laser-treatment" element={<LaserTreatmentPage />} />
          </Routes>
        </MemoryRouter>
      </MedplumProvider>
    );
  ...
  describe('Page Display', () => {
    test('should load and render treatment data', async () => {
      renderPage();
      const titles = await screen.findAllByText('Laser Treatment');
