...
export function RoomEditor(props: RoomEditorProps): JSX.Element {
  ...
  const [hasLaser, setHasLaser] = useState(
    sourceRoom?.extension?.find((e) => e.url === 'http://melissaknudson.com/fhir/StructureDefinition/has-laser')
      ?.valueBoolean ?? false
  );
  ...
  const handleSave = async (): Promise<void> => {
    try {
                ...
                coding: [
                  {
                    ...
                    code: roomType,
                    display: ROOM_TYPES.find((t) => t.code === roomType)?.label || '',
                  },
                ],
              },
            ]
      ...
      const result = await (mode === 'edit' && roomId
        ? medplum.updateResource(location as any)
        : medplum.createResource(location as any));
      showNotification({
        color: 'green',
        title: 'Success',
    ...
    <Paper p="md">
      <Stack gap="md">
        ...
        <Select
          ...
          value={roomType || ''}
          onChange={(value: any) => setRoomType(value)}
